...
public class SimpleDungeon : MonoBehaviour
{
    ...
    private void AssignSpecialRooms()
    {
        ...
        int treasureCount = Mathf.Min(Mathf.Max(1, normalRooms.Count / 4), normalRooms.Count);
    ...
    private void CreateRoomsAndCorridors()
    {
        ...
        TryAddRoom(Vector2Int.zero, startSize, RoomType.Start);

        int attempts = 0;

        while (rooms.Count < roomCount && attempts++ < maxPlacementAttempts)
        {
            List<Room> existingRooms = new(rooms.Values);        //현재 생성되어 있는 방들을 List 로 복사
    ...
    {
        List<Vector2Int> result = new(Directions);      //원본 방향 배열을 수정하지 않도록 새로운 List만든다.
    ...
    {
        //방을 중심으로 방이 시작되는 상대 좌표를 계산
        int min = -size / 2;
        int max = min + size;

        //기본 방 주변 한 칸의 여백까지 검사하여 방끼리 바로 붙지 않게 합니다.
        ...
        rooms.Add(center, room);      //방의 중심 좌표를 Key 로 사용하여 Dictionary에 방을 추가합니다.

        for(int x = min; x < max; x++)
        {
            for(int y = min; y < max; y++)
            {
                Vector2Int cell = center + new Vector2Int(x, y);       //방 중심에 상대 좌표를 더하여 실제 셀 좌표를 계산 합ㄴ디ㅏ.
                floors.Add(cell);       //해당 위치를 전체 던전의 바닥 목록에 추가
                roomFloors.Add(cell);   //해당 위치를 방 전용 바닥 목록에도 추가
            }
        }

        return true;        //정상적으로 방 생성 했으므로 true 반환.
    }
    ...
    private void CreateCorridor(Vector2Int start, Vector2Int end)       //시작 방과 끝 방 사이에 ㄱ 자 형태의 복도를 생성
    {
        ...
        {
            WalkX(ref current, end.x);  //X축으로 목표 X좌표까지 이동
            WalkY(ref current, end.y);  //Y축으로 목표 Y좌표까지 이동
        }
        else
        {
            WalkY(ref current, end.y);  //Y축으로 목표 Y좌표까지 이동
